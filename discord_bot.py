...
def _semantic_search(keyword: str, limit: int = 15) -> Optional[list[dict]]:
    ...
    if not cache:
        return None
    # 向量臂：查詢加 query: 前綴，對每個 session 取「最相似的那一塊」的分數代表該 session
...
async def cmd_confirm(interaction: discord.Interaction, switch: str):
    if not await check_auth(interaction): return
