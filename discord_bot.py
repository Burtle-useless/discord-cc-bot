...
def _get_embed_model() -> Optional["TextEmbedding"]:  # noqa: F821（fastembed 為選配，型別不在頂層 import）
    ...
    try:
        ...
        # paraphrase-multilingual-MiniLM-L12-v2：384 維、約 50 種語言含中文、onnxruntime 後端（不需 PyTorch）
        # 註：舊版的 intfloat/multilingual-e5-small 在 fastembed 0.8+ 已從支援清單移除
...
def _build_or_update_vectors() -> dict:
    ...
    # 批次 embedding（MiniLM 為對稱模型，query/passage 都不加前綴）
    if pending:
        passages = [t for _, _, t in pending]
...
def _semantic_search(keyword: str, limit: int = 15) -> Optional[list[dict]]:
    ...
    if not cache:
        return None
    qvec = list(model.embed([keyword]))[0]
